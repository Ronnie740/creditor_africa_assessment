...
const StepIndicator = ({ step }: { step: 'account' | 'shipping' | 'payment' }) => {
	...
	return (
		<div className='flex items-center mb-8 text-lg font-medium select-none'>
			<span className='text-primary'>{t('stepAccount')}</span>

			<span className='h-[2px] w-6 bg-primary/30 mx-3'></span>
			...
			<span className={cn(step !== 'account' ? 'text-primary' : 'text-gray-800')}>{t('stepShipping')}</span>
			...
			<span className={cn(step === 'payment' ? 'text-primary' : 'text-gray-800')}>{t('stepPayment')}</span>
		</div>
	);
};
